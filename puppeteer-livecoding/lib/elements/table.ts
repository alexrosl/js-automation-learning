...
class Table extends BaseElement {
    ...
    async get() {
        ...
        if (this.transformData) {
            return this.transformInitialData(data);
        }
        return data;
    }

    private transformInitialData(data) {
    ...
    private async getBodyData(headerMap) {
        const data = [];
        const bodyRows = await this.currentElement.$$('table.machines_list tbody tr');
        for (const row of bodyRows) {
            data.push(await this.getRowData(row, headerMap));
        }
        return data
    }
